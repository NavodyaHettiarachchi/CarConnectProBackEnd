...
const RepairCenter = require('../models/repair.model');
...
router.post(
  ...
  [
    ...
    try {
      const exsistingCenter = await RepairCenter.findOne({ $or: [{ username, email }] });
      if (exsistingCenter) {
        return res.status(400).json({ message: 'Center name/email already exists' });
      }

      const center = new RepairCenter({ username, name, location: { address, city, province }, contactInfo: { phone, email } });
      center.setPassword(password);
      await center.save();
      const centerData = await RepairCenter.findOne({ username });
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      const center = await RepairCenter.findOne({ username });
